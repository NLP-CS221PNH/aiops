...
from .common import (DEFAULT_CONFIG, DEFAULT_OUTPUT, IMPL, ROOT, canonical_hash, load_config,
                     read_json, read_jsonl, require, require_root, safe_path, sha256, write_json, write_jsonl)
...
def _output_path(output):
    ...
    allowed = [IMPL / '.test-work']
    if ROOT is not None:
        allowed.append(ROOT / '.test-work')
...
def _load_audit():
    ...
    return module.audit_sources(require_root())
