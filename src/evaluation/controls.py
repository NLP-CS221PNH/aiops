"""Oracle/random packers with hydrated canonical chunks. No F1 index mix."""
...
from src.evaluation.metrics import calculate_ndcg
from src.generation.context_builder import pack_context


def _as_knowledge(chunk_id: str, chunk: Mapping[str, Any] | None) -> dict[str, Any]:
    if chunk is None:
        raise ValueError(f'missing_chunk:{chunk_id}')
    text = str(chunk.get('text') or chunk.get('content') or '')
    if not text.strip():
        raise ValueError(f'missing_chunk_text:{chunk_id}')
    revision = chunk.get('source_revision')
    if not revision:
        raise ValueError(f'missing_source_revision:{chunk_id}')
    start = int(chunk.get('start_codepoint', 0))
    end = int(chunk.get('end_codepoint', start + len(text)))
    if end < start:
        raise ValueError(f'inconsistent_offsets:{chunk_id}')
    return {
        'chunk_id': chunk_id,
        'evidence_id': chunk.get('evidence_id') or chunk_id,
        'text': text,
        'source_kind': chunk.get('source_kind') or 'knowledge',
...
def pack_oracle(
    ...
    knowledge = [_as_knowledge(chunk_id, lookup.get(chunk_id)) for chunk_id in ranked]
    actual_obs, actual_know, context_hash, ledger = pack_context(list(observations or []), knowledge)
    packed_ids = [item.get('chunk_id') or item.get('evidence_id') for item in actual_know]
    ndcg = calculate_ndcg(packed_ids, dict(qrels), k)
    return {
        'condition': 'G-oracle',
        ...
        'ledger': ledger,
        'retrieval_ndcg5': ndcg.value if ndcg.status.name == 'ELIGIBLE' else None,
        'retrieval_ndcg5_status': ndcg.status.value,
        'generation_status': 'NOT_RUN',
        'diagnostic': 'upper_bound_after_packing',
    }
...
def pack_random(
    ...
    chosen = sample[:k]
    lookup = chunks_by_id or {}
    knowledge = [_as_knowledge(chunk_id, lookup.get(chunk_id)) for chunk_id in chosen]
    actual_obs, actual_know, context_hash, ledger = pack_context(list(observations or []), knowledge)
    return {
        'condition': 'G-random',
...
def answerability_slice(incident_ids: Sequence[str], labels: Mapping[str, str]) -> dict[str, Any]:
    ...
    valid = {'answerable', 'unanswerable'}
    if not any(labels.get(item) in valid for item in incident_ids):
        return {'status': 'NOT_RUN', 'reason': 'no_valid_answerability_labels'}
