...
from src.data.common import IMPL, research_pack_root
...
def git_head() -> str:
    return subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=IMPL, text=True).strip()


def git_dirty() -> bool:
    return bool(subprocess.check_output(["git", "status", "--porcelain"], cwd=IMPL, text=True).strip())
    ...
    for incident_id, ranking in rankings.items():
        ...
        for metric, result in (("ndcg_5", ndcg), ("mrr_10", mrr), ("recall_20", recall)):
            rows.append({
                ...
                "system_sha256": sha256_file(Path(system_path)),
