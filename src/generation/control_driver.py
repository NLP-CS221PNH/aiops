...
def generation_allowed(config: Mapping[str, Any]) -> bool:
    if config.get("permissions", {}).get("local") == "approved":
        return True
...
def run_control_generation(
    ...
    messages = build_messages(
        "\n".join(str(item.get("text") or "") for item in packed.get("actual_obs") or []) or "control observations missing",
        packed.get("actual_know") or [],
        condition=str(packed.get("condition") or "G-oracle"),
    )
    raw, metadata = generate_response(messages, {"incident_id": incident_id}, config)
    ...
    record["prompt_preview"] = messages[1]["content"][:500]
    return record
