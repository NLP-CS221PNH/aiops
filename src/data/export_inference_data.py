...
from .common import (
    DEFAULT_CONFIG, IMPL, DataContractError, canonical_hash, load_config,
    read_jsonl, redact, require_research_pack_root, safe_path, sha256, validate_inference, validate_record,
...
def _cleanup_staging(staging: Path, parent: Path, prefix: str) -> None:
    ...
    if staging.exists():
        ...
        if (resolved != staging.absolute() or resolved.parent != parent.resolve()
                or not resolved.is_relative_to(IMPL.resolve())
...
def export_inference_data(
    config_path: Path = DEFAULT_CONFIG, source_root: Path | None = None,
    output_dir: Path = IMPL / "data/inference",
) -> dict:
    """Validate, stage and atomically publish one immutable inference release."""
    source_root = Path(source_root) if source_root is not None else require_research_pack_root()
    ...
    try:
        ...
        validate_inference(staging, config, require_live_transform=True)
        if output.exists():
