...
from .common import (
    DEFAULT_CONFIG, IMPL, DataContractError, load_config, read_jsonl,
    require_research_pack_root, safe_path, sha256, write_json,
)
...
def export_private_sidecars(
    config_path: Path = DEFAULT_CONFIG, source_root=None,
    ...
    source_root = Path(source_root if source_root is not None else require_research_pack_root()).resolve()
    if census is None:
